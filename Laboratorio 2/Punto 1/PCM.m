# Entradas
#   m   =   Función original sobre la cual se realizará el PCM
#   Tm  =   Periodo de la función
#   k   =   Numero de veces que se muestreará la señal
#   Mp  =   Altitud máxima de la señal
#   n   =   Numero de Bits que se usará en el proceso de codificación

# Salida
#   cadena =  Cadena de unos y ceros que representan la señal en formato digital


function cadena = PCM(m,Tm,k,Mp,n)  
  
  #Se define la función para ser trabajada en octave  
  m = inline(m, "t");
  
  #Para mostrar el funcionamiento de PCM primero se graficará la función original
  t = [0:0.01:Tm];
  subplot(2,2,1);
  ...
  #Se define la frecuencia de muestreo, la cual sera k veces la frecuencia de la señal
  fs = k*fm;      
  
  #A partir de la frecuencia de muestreo se calcula el periodo de muestreo, 
  #este ayudará a calcular el intervalo sobre el cual se muestreará la señal
  Ts = 1/fs;      
  
  #Se calcula los puntos de muestreo, con ayuda del perido de muestreo y 
  #el periodo de la señal, estos puntos tendran una distancia Ts entre si
  t = [0:Ts:Tm];  
  ...
  #Se calcula el numero de franjas que serán usadas para la discretización vertical
  L = power (2, n);
  ...
  #Se lleva cabo el proceso de codificiación
  ...
  #Se quita el valor más alto utilizado anteriormente
  enc = enc(1:end-1,:);
  
  
  #Ahora se unen todos los valores en la cadena de bits que representará la señal
  cadena = "";
  for i = [1:size(enc)(1)]
    for j = [1:size(enc)(2)]
  ...
  #DEMODULACIÓN
  #Pequena demostración del proceso de demodulación
